...
async function enableUser(userId: string) {
  ...
  try {
    ...
    const updatedUserData = {
      ...userData,
      isDisabled: false,
      enabledAt: new Date().toISOString(), // 记录启用时间，可选
    };
...
export async function handleEnableUser(req: Request, userId: string) {
  ...
  try {
    ...
    return createErrorResponse("Failed to enable user");
  }
}

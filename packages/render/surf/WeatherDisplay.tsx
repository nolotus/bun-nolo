			...
			{isLoading ? (
				...
				<>
					...
					>
						...
						<div className={headerCellStyle}>水温(°C):</div>
						<div className={headerCellStyle}>风速(m/s):</div>
						<div className={headerCellStyle}>阵风(m/s):</div>
						<div className={headerCellStyle}>气温(°C):</div>
						<div className={headerCellStyle}>云量(%):</div>
						<div className={headerCellStyle}>降水(mm):</div>
					</div>
							...
							{Object.entries(groupedWeatherData).map(([monthDay, hours]) => (
								<React.Fragment key={monthDay}>
									<div key={monthDay} className="flex flex-col">
										{/* 日期列 */}
										<div className={clsx(stickyCellStyle, "bg-blue-50")}>
											<div
												className={clsx(
													dateCellStyle,
													fontBoldStyle,
											...
											{hours
												...
												.map((hour, hourIndex) => (
													...
													>
														...
														<div className="text-gray-600 text-sm h-6">
															{getDataByMode(hour, "waterTemperature")}
														</div>
														...
														<div className="text-gray-600 text-sm h-6">
															{getDataByMode(hour, "precipitation")}
														</div>
													</div>
												))}
										</div>

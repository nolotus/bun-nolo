...
/** 合并多个 ref（floating-ui ref + 外部 ref） */
...
function Dropdown<T = any>(props: DropdownProps<T>) {
  const {
    items = [],
    onChange,
    placeholder,
  ...
  const getItemLabel = (item: T): string =>
    String((item as any)?.[labelField as any] ?? "");

  const getItemValue = (item: T, index: number): React.Key =>
    (item as any)?.[valueField as any] ?? index;
  ...
  const referenceRef = mergeRefs<HTMLButtonElement>(refs.setReference, ref);
                ...
                <div role="listbox" className="dropdown-list">
                  {loading ? (
                    ...
                    filteredItems.map((item, index) => {
                      ...
                      const key = getItemValue(item, index);
